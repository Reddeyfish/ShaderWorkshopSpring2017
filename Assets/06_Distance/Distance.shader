Shader "Custom/Distance" {
    ...
    Properties {
        ...
        _Point("Origin Point of transparency falloff", Vector) = (0, 0, 0, 0)
	}
        ...
        Pass
        {
            ...
            CGPROGRAM
                ...
                fixed4 frag (v2f i) : SV_Target
                {
                    ...
                    fixed4 textureCol = tex2D(_MainTex, i.uv % 1);
                    //sample from the current texture

                    fixed4 outputColor = _Color * textureCol; //tint the color
                    float dist = distance(i.wpos, _Point); //distance from a point
                    outputColor.a *= (1 - dist); //more opaque when close to the point
